...
Container.prototype.destroy = function() {
	return this.stop()
		.then(() => {
			return this._client._request('DELETE', '/containers/'+this.name);
		});
};
...
Container.prototype.update = function(data) {
	return this.info()
		.then(info => _.merge(info, data))
		.then(data => this._client._request('PUT', '/containers/'+this.name, data))
		.then(() => this.info());
};
...
Container.prototype.add_disk = function(name, source, path) {
	...
	// TODO make sure source is owned by lxd_data user
