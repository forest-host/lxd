...
Container.prototype.exec = function(cmd, args, options) {
	...
	// Add options to defaults, overwriting defaults on double keys
	var defaults = {
		timeout: 60 * 10 * 1000,
	};
	...
	return this.client.run_async_operation({ method: 'POST', path: '/containers/'+this.name+'/exec', data, timeout: options.timeout });
};
