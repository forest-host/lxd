...
var Client = function(cert_path, key_path) {
	this._key = fs.readFileSync(key_path);
	this._cert = fs.readFileSync(cert_path);

	// TODO pass this as arguments (config pattern)
	this._host = '10.0.0.2';
	this._port = '8443';
	this._websocket = 'ws+unix:///var/lib/lxd/unix.socket';
	this._api_version = '1.0';
};
...
Client.prototype._make_request = function(method, url, data) {
	return new Promise((resolve, reject) => {
		url = 'https://' + this._host + ':' + this._port + '/' + this._api_version + url;
...
Client.prototype.containers = function() {
