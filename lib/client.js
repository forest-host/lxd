 ...
 * TODO - First test if we can just build on ceph by comparing the times of building on cephfs vs building on ssd
 * TODO - Add query string support so we can pass target if build time differs to much
 */
Client.prototype.get_request_config = function(method, path, data) {
	return {
		// TODO - pass target in query string
